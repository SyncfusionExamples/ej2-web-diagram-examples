// Syncfusion EJ2 Diagram - Collaborative Editing Sample
let diagram;
let palette;
let hubConnection;

// SignalR Configuration
let roomName = "ej2_aspnetcore_group";
...
async function OnConnectedAsync(connId) {
    if (connId) {
...
// Placeholder for peer selection change handling
function peerSelectionChanged(evt) {
    console.log("Peer selection changed:", evt);
}

// Update user count on header
...
hubConnection.on("OnSaveDiagramState", async function(requestId) {
    let data = diagram.saveDiagram();
    await hubConnection.invoke("ProvideDiagramState", requestId, data);
});

hubConnection.on("LoadDiagramData", function(remoteData) {
...
hubConnection.on("UpdateVersion", function(serverVersion) {
    clientVersion = serverVersion;
});

hubConnection.on("ReceiveData", function(data, serverVersion, evt) {
    ReceiveData(data);
    clientVersion = serverVersion;
    peerSelectionChanged(evt);
});

hubConnection.on("CurrentUsers", function(users) {
...
function onHistoryChange(args) {
    ...
    let changes = diagram.getDiagramUpdates(args);
...
function getFlowShape(id, shapeType) {
    return { id: id, shape: { type: 'Flow', shape: shapeType } };
}

// Get basic shape
function getBasicShape(id, shapeType) {
    return { id: id, shape: { type: 'Basic', shape: shapeType } };
}
...
function initializeSymbolPalette() {
    ...
    palette = new ej.diagrams.SymbolPalette({
        expandMode: 'Multiple',
        palettes: palettes,
        width: paletteContainer.offsetWidth || '230px',
