...
// Helper function to get image path for preview (using Vite's URL import)
const getShapeImagePath = (paletteId: string): string => {
    try {
        // Use new URL constructor with import.meta.url for Vite compatibility
        return new URL(`../assets/Shapes/${paletteId}.png`, import.meta.url).href;
    } catch (error) {
        // Return flow image as default fallback for any missing images
        try {
            return new URL(`../assets/Shapes/flow.png`, import.meta.url).href;
        } catch (fallbackError) {
            return '';
        }
    }
...
interface PaletteMetadata {
    id: string;
    title: string;
    displayName: string; // Full display name for property panel
    iconCss: string;
    enabled: boolean;
    isActive: boolean; // Whether this palette is currently displayed in simple palette
    isCustom: boolean; // Distinguish between predefined and custom palettes
    assetsPath?: string; // Path to shape assets (for future image loading)
    gitLink?: string;
    docsLink?: string;
    shapes: (NodeModel | ConnectorModel)[];
    lastEnabledTimestamp?: number; // Track when palette was last enabled
}
...
export const AdvancedSymbolPalette = () => {
    const floorplans: FloorplanShapes = new FloorplanShapes();
    const networkShapes: NetworkShapes = new NetworkShapes();
    const paletteRef = useRef<SymbolPaletteComponent>(null);
    const manageDialogRef = useRef<DialogComponent>(null);
    // ====== STATE MANAGEMENT ======

    // All available palettes (predefined + custom)
    // Flow is enabled by default
    const [palettes, setPalettes] = useState<PaletteMetadata[]>([
        {
            id: 'flow',
            title: 'Flow',
        ...
        // },
        {
            id: 'native',
            title: 'Native',
            displayName: 'Native SVG Shapes',
            iconCss: 'e-icons e-custom',
            enabled: false,
            isActive: false,
            isCustom: false,
            shapes: createNativeShapes()
        },
        {
            id: 'swimlane',
            title: 'Swimlane',
            displayName: 'Swimlane Shapes',
            iconCss: 'e-icons e-table',
            enabled: false,
            isActive: false,
            isCustom: false,
            shapes: createSwimlaneShapes()
        },
        {
            id: 'network',
            title: 'Network',
            ...
            isActive: false,
            isCustom: false,
            shapes: networkShapes.getNetworkShapes()
        },
        {
            id: 'floorplan',
            title: 'Floorplan',
            ...
            isActive: false,
            isCustom: false,
            shapes: floorplans.getFloorPlans()
        }
    ]);
    ...
    // Preview image state - default to flow
    const [previewImage, setPreviewImage] = useState('flow');
    ...
    // HTML-style helper shapes removed. Use `HTMLTemplate()` which returns safe Native SVG nodes for the palette.

    function createNativeShapes(): NodeModel[] {
        const nativeShapes: NodeModel[] = [
            {
                id: 'NativeNode1',
                width: 80,
                height: 80,
    ...
    // function HTMLTemplate(): NodeModel[] {
    //     // SymbolPalette cannot reliably render `HTML` shaped nodes in all environments.
    //     // Use lightweight `Native` SVG nodes that visually match the HTML templates.
    ...
    //     return '';
    // }
    function createSwimlaneShapes(): NodeModel[] {
    ...
    /**
     * Generate all enabled palettes for display
     * Shows ALL enabled palettes in the symbol palette (like Syncfusion reference)
     */
    ...
    // Maintain a slightly-debounced activePalettes state to avoid
    // Syncfusion SymbolPalette internal timing issues during rapid updates.
    const [activePalettes, setActivePalettes] = useState(getActivePalettes());

    useEffect(() => {
        const t = setTimeout(() => {
            setActivePalettes(getActivePalettes());
        }, 50);
    ...
    const handleManagePalettesClick = (e: React.MouseEvent) => {
        e.preventDefault();
    ...
    /**
     * Handle palette enable/disable via checkbox
     * Simple toggle - all enabled palettes are shown
     * Also updates the preview image
     */
    const handlePaletteToggle = (paletteId: string, args: ChangeEventArgs) => {
        const isChecked = args.checked ?? false;

        // Update preview image to the selected palette when checked
        if (isChecked) {
            setPreviewImage(paletteId);
        }
    ...
    return (
        <div style={{
            ...
            <div style={{
                ...
                onClick={handleManagePalettesClick}
            >
                <a href="#" style={{
                    fontSize: '13px',
                    color: '#007bff',
            ...
            >
                <div style={{ display: 'flex', height: 'calc(100% - 60px)', padding: 0 }}>
                    {/* Left Panel - Palette Selection */}
                    <div style={{
                        width: '180px',
                        borderRight: '1px solid #e0e0e0',
                        padding: '16px',
                        ...
                        {/* Shape Categories - Simple Checkboxes */}
                            ...
                            {palettes.map(palette => (
                                <div key={palette.id}>
                                    <CheckBoxComponent
                                        ...
                                        change={(args) => handlePaletteToggle(palette.id, args)}
                    ...
                    <div style={{
                        ...
                        {previewImage ? (
                            <img
                                key={previewImage} // Force re-render when preview changes
                                id="shapePreviewImage"
                                src={getShapeImagePath(previewImage)}
                                alt={`${previewImage} shapes preview`}
                                style={{
                                    maxWidth: '100%',
                                    objectFit: 'contain',
                                    padding: '16px',
